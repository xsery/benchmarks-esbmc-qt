...
{	__ESBMC_atomic_begin();
	...
	unsigned int grid_position = (unsigned int)
			indexOfBlock[id].x + indexOfBlock[id].y*gridDim.x +
	...
	thread_index.y = (unsigned int) ( (linear_value % (blockDim.x*blockDim.y))
			/ blockDim.x );
	thread_index.x = (unsigned int) ( (linear_value % (blockDim.x*blockDim.y))
			% blockDim.x );
...
uint3 getBlockIdx(unsigned int id)
{
	...
	unsigned int linear_value = (unsigned int)
			id  / (blockDim.x*blockDim.y*blockDim.z);

	uint3 block_index;
	block_index.z = (unsigned int) (linear_value / (gridDim.x * gridDim.y));
	block_index.y = (unsigned int) ( (linear_value % (gridDim.x*gridDim.y))
				/ gridDim.x );
	block_index.x = (unsigned int) ( (linear_value % (gridDim.x*gridDim.y))
				% gridDim.x );
